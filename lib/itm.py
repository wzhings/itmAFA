...
from lib.loss import ContrastiveLoss_mixup

import logging

logger = logging.getLogger(__name__)

class ITMModel(object):
    """
        The standard ITM model (VSE-based)
    """
    ...
    def __init__(self, opt):
        ...
        self.txt_enc = get_text_encoder(opt.embed_size, opt=opt, no_txtnorm=opt.no_txtnorm)
        if torch.cuda.is_available():
            self.img_enc.cuda()
            self.txt_enc.cuda()
            cudnn.benchmark = True  

        self.criterion = ContrastiveLoss_mixup(opt=opt,
                                         margin=opt.margin,
    ...
    def forward_emb(self, images, captions, lengths, image_lengths=None):
        ...
        if self.opt.precomp_enc_type == 'basic':
            ...
            cap_emb, bert_out = self.txt_enc(captions, lengths)
        else:
            if torch.cuda.is_available():
                images = images.cuda()
                captions = captions.cuda()
        ...
        # return img_emb, cap_emb, img_clip, cap_clip
        return img_emb, cap_emb


    def forward_emb_clip(self, images, captions, lengths, img_clip, cap_clip, vit, image_lengths=None):
        """
        revise: probabaly not work, contain bugs
        Compute the image and caption embeddings
        """
        # Set mini-batch dataset  
        if self.opt.precomp_enc_type == 'basic':
            if torch.cuda.is_available():
                images = images.cuda()
                captions = captions.cuda()
                img_clip = l2norm(img_clip, dim=-1).cuda()
                cap_clip = l2norm(cap_clip, dim=-1).cuda()
                vit = vit.cuda()
                image_lengths = image_lengths.cuda()
                lengths = torch.Tensor(lengths).cuda()
            #print(f"check cap imgs shape: {images.shape}.")
            cap_emb, bert_out = self.txt_enc(captions, lengths, images, vit, image_lengths)
            img_emb = self.img_enc(images, image_lengths, vit, bert_out, lengths)
        else:
            if torch.cuda.is_available():
                images = images.cuda()
                captions = captions.cuda()
                ...
                image_lengths = image_lengths.cuda()
            img_emb = self.img_enc(images)
            #print(f"check cap length type: {type(lengths)}.")
            cap_emb, bert_out = self.txt_enc(captions, lengths, images, image_lengths) # need to be changed here!
